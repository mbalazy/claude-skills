...
- **`scripts/sim-ui.sh`** - the driver for everything: observation and interaction via WebDriverAgent's HTTP API (localhost:8100), lifecycle via `xcrun simctl`. No MCP server needed. Run it with no args for the command list. It auto-starts WDA (`com.facebook.WebDriverAgentRunner.xctrunner` must be installed on the sim). **A physical iPhone on the cable is the same driver**: put the phone's udid in `SIM_UDID` and every command works unchanged - see "A physical iPhone is the same loop" below.
- **`scripts/wda-device.sh`** - brings WebDriverAgent up on a physical iPhone (and reports or stops it); the one piece of the phone's plumbing that cannot be automatic.
...
- **In a throwaway `git worktree`, none of this exists locally.** Both the skill directory and `.simulator-verify/` are typically git-ignored, so a fresh worktree of the same repo has neither - and no amount of re-running will create them. Call the scripts by their absolute path from wherever the skill is installed and point them at the tree under test: `<skill-dir>/scripts/rig-check.sh --repo <worktree>`. `rig-check.sh` then reads the project config from the repo's MAIN worktree by itself, so `--bundle-id` is only needed when even that has no config. For `sim-ui.sh` in a hand session, export `SIM_UDID` and `WDA_PORT` - it derives nothing.
- **Inside an executor run (a `pm run-epic` / `pm work` worker) the slot env is ALREADY set** - `SIM_UDID`, `WDA_PORT` and `ADDITIONAL_METRO_PORT` are in your process (`printenv SIM_UDID` shows them). Do NOT `export` them and do NOT prefix a command with `SIM_UDID=... scripts/sim-ui.sh`: the worker's allowlist is a literal prefix match on the command you type, and both forms are refused. Call the scripts by the absolute path the `## Runtime rig` section of your prompt lists - `<your worktree>/.claude/skills/simulator-verify/scripts/sim-ui.sh ...` - never `sh <script>` / `bash <script>` (a different prefix, refused). `xcrun simctl`, `curl`, `python3` and `printenv` are allowed under their plain names.
...
## A physical iPhone is the same loop (verified 2026-09-25, iPhone 12 / iOS 27)

Everything above and below applies to a phone on the USB cable, with the same scripts and the same commands. The scripts tell a phone from a simulator by **the shape of the udid** (`0000XXXX-<16 hex>` from `xcrun devicectl list devices`, against a simulator's 8-4-4-4-12), never by a flag somebody has to remember, and switch the plumbing underneath:

| | simulator | physical iPhone |
|---|---|---|
| identity | `SIM_UDID=<sim udid>` | `SIM_UDID=<phone udid>` (same variable; with no simulator booted and one phone plugged in it is picked by itself) |
| lifecycle (`launch`/`relaunch`/`terminate`/`openurl`) | `xcrun simctl` | `xcrun devicectl` (`process launch --terminate-existing <bundle> -- <args>`, the `--` keeps devicectl from eating `-RCT_jsLocation`; `process openURL <url>`; terminate by pid) |
| WebDriverAgent | xctrunner on the sim, auto-started, port owner = the sim's process path | `xcodebuild test` on the phone + an `iproxy <port> 8100 -u <udid>` tunnel, brought up by **`scripts/wda-device.sh <udid> --team <id>`** (never auto-started - it is a minutes-long test run); `WDA_PORT` derives itself from the tunnel that names the udid; a sim's port is refused with the tunnel's port named |
| geometry / bounds check | `simctl getenv` | WDA `/wda/screen` |
| `screenshot` | `simctl io` | `devicectl device capture screenshot` (WDA's own `/screenshot` returned a stale frame on a device) |
| `type` | one request per string | **one character per request, 0.35 s apart** - a burst drops characters (`5125550199` arrived as `(519) 9`); `SIMUI_TYPE_DELAY` forces this on a sim too |
| `rig-check.sh` | marker read from the simulator log | marker read over Metro's CDP inspector (`read-rn-logs-cdp.py --device iPhone`); the launch carries `-RCT_jsLocation <mac-usb-address>:<port>` because the phone cannot reach `localhost` |
| JS console | `read-rn-logs-cdp.py` | the same, with `--device iPhone` - a phone registers on Metro as plain `iPhone`, and the reader prefers that exact name over the "iPhone 17" substring matches |

`rig-check.sh --udid <phone udid>` is the same gate as on a simulator and passes in ~12 s. What it cannot see on a phone it says instead of guessing: whether the binary embeds a `main.jsbundle`, and when it was built. The Mac's address on the USB link (`169.254.x.x`) is derived from the phone's existing connection to Metro, else from the only link-local interface; `--host` overrides.

Two device-only traps, both silent:
- **A fresh install waits on the iOS "Local Network" prompt.** The first launch after every fresh install sits on the launch screen with zero requests to Metro and zero errors anywhere until a human taps Allow on the phone. `rig-check.sh` names it when the phone holds no connection to Metro.
- **`-RCT_jsLocation` loses to `ip.txt` only when that file lists MORE than one candidate.** A device build carries `ip.txt` with the Mac's address(es); this app's `AppDelegate.bundleURL()` probes the candidates only when there are several, and with a single line (the usual case) the launch flag wins - measured: the phone loaded from `169.254.190.191:8081` with `ip.txt` saying `192.168.0.110`.

The per-repo `config.md` records the phone's udid, the Apple team whose Xcode-managed profile lists it, the runner bundle id, the tunnel port and the build recipe (a device build needs a provisioning profile, so the bundle id and configuration may differ from the simulator's).
...
**3. An HTTP probe, when the log channel is not usable.** `console.log` is only worth instrumenting if you can read it back. On a **physical device** `idevicesyslog` never relays it (New Architecture + Hermes send JS logs to the DevTools inspector only), but the CDP tap above reads them as long as the phone loads its JS from this Metro (`read-rn-logs-cdp.py --device iPhone`, verified 2026-09-25); a phone running an embedded bundle has no inspector target at all. The other case for a probe is a large payload on any target - a whole element tree or a serialized object gets truncated and line-wrapped into something you cannot parse.
...
**Before an auth, OTP, SMS or payment flow, find out which backend the BUILD talks to - not which one the repo's `.env` names today.** A React Native build bakes its API base URL in at compile time, so the installed binary can be pointing somewhere the current `.env` no longer does, and the answer changes what the flow even means: against a local rig, a verification code is usually a fixed constant and NO message is ever sent, so waiting for one is waiting forever. Read the value the build actually carries, then ask the backend rather than the app:

```sh
grep -E '^API_URL|^BASE_URL' <the tree the build came from>/.env
curl -s -X POST <base>/auth/send-code -H 'Content-Type: application/json' -d '{"phoneNumber":"+1..."}'
```

A reply along the lines of `{"success":true,"message":"Mocked: Verification code sent successfully"}` is the whole answer: nothing was sent, and the code is whatever that environment fixes it to. Measured 2026-08-28 - a whole session was spent asking a human to read an SMS that a mocked backend was never going to send, with the fixed code already written down in a memory nobody thought to consult, because the question "which backend is this build on?" was never asked.
...
```
then Read the PNG. You lose the element tree and tap control (observe-only), but screenshot + Read is enough for a visual compare loop. Lifecycle commands (`launch`/`relaunch`/`terminate`/`openurl`/`screenshot`) never need WDA. On a physical iPhone the fallback is the same (`sim-ui.sh screenshot` goes through devicectl and needs no WDA), and the fix is `scripts/wda-device.sh <udid> --status` / a fresh `wda-device.sh <udid> --team <id>`. The `mobile-mcp` MCP server is the legacy path (same WDA backend, less control) - removed from the project config 2026-07-06 and not needed for devices either.
...
- **On a physical device the same flag works, with one documented exception.** A device build carries an `ip.txt` inside its bundle holding the Mac's address (`react-native-xcode.sh` writes it only when `PLATFORM_NAME` is not a simulator - which is why a simulator build never has one). Stock `RCTBundleURLProvider` prefers `jsLocation` over that file but drops it whenever its own packager-reachability probe fails; an app with a custom `bundleURL()` in its AppDelegate may read `ip.txt` FIRST and overwrite `jsLocation` - this app does so only when `ip.txt` lists MORE than one candidate. With a single line the flag wins (measured 2026-09-25: loaded from the USB address with `ip.txt` naming the WiFi one). `rig-check.sh` passes the flag on a phone by itself, with the Mac's USB address; before relying on it in another app, read that app's `bundleURL()`.
- **Two builds of the app on one simulator share the URL scheme, and `openurl` picks one of them without saying which.** On 2026-09-25 `com.littleengine.development` and `com.littleengine.app` (a Local-rig build) were both installed; `simctl openurl littleengine://debug/nav/BusinessHours` opened the sheet in the DEV app while the session believed it was driving the Local one - three saves went to the wrong backend and 40 minutes went into a phantom "Save does nothing" bug. With more than one build installed (`xcrun simctl listapps <udid> | grep CFBundleIdentifier`), never `openurl`: launch the target by bundle id and navigate through the UI, and before reading any result confirm WHICH app answered - by an account-specific element in the tree (the avatar initials, the profile name) or by the pid in `xcrun simctl spawn <udid> launchctl list | grep <bundle-prefix>`. The CDP tap has the same ambiguity: pin it with `--app <bundle id>`, both apps share the Metro.
...
- **A registered URL scheme is not proof `openurl` reaches JS, and there are TWO silent ways it does not.** Both were measured on the same app, 2026-08-27 and 2026-08-28, and either one alone looks exactly like dead handler code.
  1. **iOS asks for confirmation first.** The first custom-scheme URL opened from outside an app raises a system alert (**"Open in 'App'?"**, Cancel / Open), and until it is accepted the URL is delivered nowhere - not to the scene delegate, not to native code, not to JS. `xcrun simctl openurl` still exits 0, and nothing in the terminal hints at it. Measured as once per app per simulator, remembered across relaunches and across installing a new build. So a deep link belongs in a batch with an alert drain right behind it: `$S do 'openurl <url>' '?alert accept --all' 'wait 2'` - the `?` keeps the runs with no alert from aborting the batch.
  2. **`RCTAppDelegate` may not forward the URL at all.** In react-native 0.83 neither `RCTAppDelegate.mm` nor its header mentions `openURL`, so an AppDelegate whose `application(_:open:options:)` only calls `super` returns false and delivers nothing; `RCTLinkingManager.application(_:open:options:)` has to be called explicitly. Confirmed in one run with a native log on both lines: `super handled=false`, `RCTLinkingManager handled=true`, and the JS listener fired for the first time.
  So before building any workflow on a deep link, prove ONE link end to end. Order the diagnosis by cost: screenshot first (the confirmation alert is right there on screen), then a `fetch` probe (3c technique 3) at listener-install time and inside the handler to separate "never delivered" from "handler not installed", then a native log to find which layer drops it. Getting this right is worth the trouble - a working deep link replaces a multi-tap reach with one command, measured at 0.2s against 16.1s for the four taps.
- **To clear a text field, type backspaces - do not tap the keyboard's Delete key by coordinate.** `$S type "$(printf '\b\b\b\b\b\b')"` empties it reliably (the field falls back to its placeholder, which is how you confirm); tapping the Delete key's rect from `elements --all` silently did nothing on one screen and worked on another, and the failure looks like a field that will not clear. Then ALWAYS read the field back before submitting: a half-cleared field plus a fresh `type` produces a plausible wrong value (a phone field showed `(555) 010-0435` after what looked like a clean retype, and a US-masked field turned a 9-digit foreign number into a 9-digit US one that the backend cheerfully accepted).
- **A segmented code input (one box per digit) loses characters when typed in one go.** `type 123456` into a six-box OTP field left `4 5 6 1` on screen - each keystroke moves focus, and a fast burst races it. Tap the on-screen keypad digit by digit with ~0.5s between taps, or type one character per call, and screenshot to confirm before submitting.

...
- **WDA refuses to start on a simulator booted without a Simulator.app window**, and also when three or more simulators are booted at once. Building a dedicated sim alongside a slot pool puts you in that state by default. Not fixed - if the runner will not come up, check the window and the booted count first.
